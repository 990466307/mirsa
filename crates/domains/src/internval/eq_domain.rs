...
impl<'tcx> EqDomain<'tcx> {
    ...
    pub fn equivalent_to(&self, other: &EqDomain<'tcx>) -> bool {
        ...
        // Canonicalize partition labels by first-seen class root order on the same vars list.
        // This compares semantic equivalence classes without depending on union-find tree shape.
    ...
    // Partial order used by abstract interpretation:
    // self <= other iff every equality known in `other` is also known in `self`.
